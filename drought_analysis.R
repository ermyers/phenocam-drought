...
# Yearly GCC Amplitude as a % of mean amplitude vs. Mean VPD Anomaly
facet_peak <- ggplot(data=filter(temp_data, Primary_Veg_Type=="AG" | Primary_Veg_Type=="EN" | Primary_Veg_Type=="DB" | Primary_Veg_Type=="GR" | Primary_Veg_Type=="SH"), aes(x=Mean_VPD_Anomaly,y=Amplitude_GCC_yearly_percent_of_mean)) +
  geom_point() +
  geom_smooth(method = lm) +
  facet_wrap(~Primary_Veg_Type,ncol=2) +
  ggtitle("Relative Yearly GCC Amplitude vs. Mean VPD Anomaly")
facet_peak
...
# Cumulative Yearly GCC as a % of mean cumulative GCC vs. Mean VPD Anomaly
facet_peak <- ggplot(data=filter(temp_data, Primary_Veg_Type=="AG" | Primary_Veg_Type=="EN" | Primary_Veg_Type=="DB" | Primary_Veg_Type=="GR" | Primary_Veg_Type=="SH"), aes(x=Mean_VPD_Anomaly,y=Cumulative_GCC_yearly_percent_of_mean)) +
  geom_point() +
  geom_smooth(method = lm) +
  facet_wrap(~Primary_Veg_Type,ncol=2) +
...
# # Define a function to generate linear correlations
# phen_usdm_corr <- function(veg_types, input_dataset, phen_metric, drought_index, drought_metric){
#   output_df <- data.frame()
#   for (veg_type in veg_types){
#     temp_model <- lm(phen_metric ~ drought_metric, filter(input_dataset, Primary_Veg_Type==veg_type))
#     new_row <- data.frame("Veg_Type" = veg_type,
#                           "Phen_Metric" = as.character(phen_metric),
#                           "Drought_Index" = as.character(drought_index),
#                           "Drought_Metric" = as.character(drought_metric),
#                           "Slope" = coef(summary(temp_model))[as.character(drought_metric),"Estimate"],
#                           "Error" = coef(summary(temp_model))[as.character(drought_metric),"Std. Error"],
#                           "Adjusted_R2" = summary(temp_model)$adj.r.squared,
#                           "F_Statistic" = summary(temp_model)$fstatistic[[1]],
#                           "P_Value" = coef(summary(temp_model))[as.character(drought_metric),"Pr(>|t|)"])
...
###
# Relative Yearly GCC Amplitude vs. Mean 1-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(Amplitude_GCC_yearly_percent_of_mean ~ Mean_SPEI_1month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "Amplitude_GCC_yearly_percent_of_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_1month",
...
# Relative Yearly Cumulative GCC vs. Mean 1-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(Cumulative_GCC_yearly_percent_of_mean ~ Mean_SPEI_1month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "Cumulative_GCC_yearly_percent_of_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_1month",
...
# SOS difference from mean vs. Mean 1-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(SOS_25_difference_from_mean ~ Mean_SPEI_1month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "SOS_25_difference_from_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_1month",
...
###
# Relative Yearly GCC Amplitude vs. Mean 3-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(Amplitude_GCC_yearly_percent_of_mean ~ Mean_SPEI_3month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "Amplitude_GCC_yearly_percent_of_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_3month",
...
# Relative Yearly Cumulative GCC vs. Mean 3-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(Cumulative_GCC_yearly_percent_of_mean ~ Mean_SPEI_3month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "Cumulative_GCC_yearly_percent_of_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_3month",
...
# SOS difference from mean vs. Mean 3-Month SPEI
for (veg_type in veg_type_list){
...
###
# Relative Yearly GCC Amplitude vs. Mean 6-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(Amplitude_GCC_yearly_percent_of_mean ~ Mean_SPEI_6month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "Amplitude_GCC_yearly_percent_of_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_6month",
...
# Relative Yearly Cumulative GCC vs. Mean 6-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(Cumulative_GCC_yearly_percent_of_mean ~ Mean_SPEI_6month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "Cumulative_GCC_yearly_percent_of_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_6month",
...
# SOS difference from mean vs. Mean 6-Month SPEI
for (veg_type in veg_type_list){
...
###
# Relative Yearly GCC Amplitude vs. Mean 12-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(Amplitude_GCC_yearly_percent_of_mean ~ Mean_SPEI_12month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "Amplitude_GCC_yearly_percent_of_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_12month",
...
# Relative Yearly Cumulative GCC vs. Mean 12-Month SPEI
for (veg_type in veg_type_list){
  temp_model <- lm(Cumulative_GCC_yearly_percent_of_mean ~ Mean_SPEI_12month, filter(temp_data, Primary_Veg_Type==veg_type))
  new_row <- data.frame("Veg_Type" = veg_type,
                        "Phen_Metric" = "Cumulative_GCC_yearly_percent_of_mean",
                        "Drought_Index" = "SPEI",
                        "Drought_Metric" = "Mean_SPEI_12month",
...
# SOS difference from mean vs. Mean 12-Month SPEI
for (veg_type in veg_type_list){
