...
# Note 10/3/24 - need to put in a check for discontinuous data for drought period calculation
...
# Attach ecoregion information
phen_with_ecoregion_df <- select(phen_with_ecoregion_df, "site", "ecoregion", "NA_L1CODE", "NA_L1NAME")
drought_statistics <- left_join(drought_statistics,phen_with_ecoregion_df,by=join_by("Phenocam"=="site"))
...
p_etf <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="EASTERN TEMPERATE FORESTS"), aes(x=Number_Weeks)) +
  geom_histogram(binwidth=2) +
  expand_limits(x = 200) +
  ggtitle("EASTERN TEMPERATE FORESTS")
p_gp <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="GREAT PLAINS"), aes(x=Number_Weeks)) +
  geom_histogram(binwidth=2) +
  expand_limits(x = 200) +
  ggtitle("GREAT PLAINS")
p_nad <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="NORTH AMERICAN DESERTS"), aes(x=Number_Weeks)) +
  geom_histogram(binwidth=2) +
  expand_limits(x = 200) +
  ggtitle("NORTH AMERICAN DESERTS")
p_nf <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="NORTHERN FORESTS"), aes(x=Number_Weeks)) +
  geom_histogram(binwidth=2) +
  expand_limits(x = 200) +
  ggtitle("NORTHERN FORESTS")
p_nfm <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="NORTHWESTERN FORESTED MOUNTAINS"), aes(x=Number_Weeks)) +
  geom_histogram(binwidth=2) +
  expand_limits(x = 200) +
  ggtitle("NW FORESTED MOUNTAINS")
p_ts <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="TEMPERATE SIERRAS"), aes(x=Number_Weeks)) +
  geom_histogram(binwidth=2) +
  expand_limits(x = 200) +
  ggtitle("TEMPERATE SIERRAS")

(p_etf + p_gp) / (p_nad + p_nf) / (p_nfm + p_ts)

q_etf <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="EASTERN TEMPERATE FORESTS"), aes(x=Sum_USDM)) +
  geom_histogram(binwidth=10) +
  expand_limits(x = 500) +
  ggtitle("EASTERN TEMPERATE FORESTS")
q_gp <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="GREAT PLAINS"), aes(x=Sum_USDM)) +
  geom_histogram(binwidth=10) +
  expand_limits(x = 500) +
  ggtitle("GREAT PLAINS")
q_nad <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="NORTH AMERICAN DESERTS"), aes(x=Sum_USDM)) +
  geom_histogram(binwidth=10) +
  expand_limits(x = 500) +
  ggtitle("NORTH AMERICAN DESERTS")
q_nf <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="NORTHERN FORESTS"), aes(x=Sum_USDM)) +
  geom_histogram(binwidth=10) +
  expand_limits(x = 500) +
  ggtitle("NORTHERN FORESTS")
q_nfm <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="NORTHWESTERN FORESTED MOUNTAINS"), aes(x=Sum_USDM)) +
  geom_histogram(binwidth=10) +
  expand_limits(x = 500) +
  ggtitle("NW FORESTED MOUNTAINS")
q_ts <- ggplot(data = filter(drought_statistics, Sum_USDM>0 & NA_L1NAME=="TEMPERATE SIERRAS"), aes(x=Sum_USDM)) +
  geom_histogram(binwidth=10) +
  expand_limits(x = 500) +
  ggtitle("TEMPERATE SIERRAS")

(q_etf + q_gp) / (q_nad + q_nf) / (q_nfm + q_ts)
