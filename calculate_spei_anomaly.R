...
for (i in 1:nrow(unique_siteyears)){
  ...
  new_row <- data.frame("Phenocam" = site,
                        "Year" = year,
                        "Max_SPEI_1month" = max(temp_df$SPEI_1month, na.rm=TRUE),
                        "Max_SPEI_3month" = max(temp_df$SPEI_3month, na.rm=TRUE),
                        "Max_SPEI_6month" = max(temp_df$SPEI_6month, na.rm=TRUE),
                        "Max_SPEI_12month" = max(temp_df$SPEI_12month, na.rm=TRUE),
...
spei_statistics <- spei_statistics %>%
  mutate(Cumulative_SPEI_1month=replace(Cumulative_SPEI_1month, Number_of_Weeks<52, NA)) %>%
  mutate(Cumulative_SPEI_3month=replace(Cumulative_SPEI_3month, Number_of_Weeks<52, NA)) %>%
  mutate(Cumulative_SPEI_6month=replace(Cumulative_SPEI_6month, Number_of_Weeks<52, NA)) %>%
  mutate(Cumulative_SPEI_12month=replace(Cumulative_SPEI_12month, Number_of_Weeks<52, NA))
